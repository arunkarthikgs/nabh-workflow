...
export default function LoginGate({ onLogin, onShowRegister }) {
  ...
  async function submit(event) { event.preventDefault(); const normalized = userId.trim().toLowerCase(); const account = accounts[normalized]; if (account && account[0] === password) { onLogin({ role: account[1] }); return; } if (!normalized.endsWith("-admin")) { setError("Invalid credentials."); return; } const response = await fetch("/api/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: normalized, password }) }); const data = await response.json(); if (!response.ok) { setError(data.error || "Invalid credentials."); return; } onLogin(data.session); }
