...
export default function LoginGate({ onLogin, onShowRegister }) {
  ...
  async function requestReset(event) { event.preventDefault(); setResetting(true); setResetMessage(""); setError(""); try { const response = await fetch("/api/forgot-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: resetIdentifier.trim(), email: resetIdentifier.trim() }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Unable to request a password reset."); setResetMessage(data.message); } catch (requestError) { setError(requestError.message); } finally { setResetting(false); } }
  return <main className="login-main"><section className="login-intro"><div className="login-mark"><ShieldCheck size={20} /></div><p className="eyebrow">NABH compliance workspace</p><h1>Documents that stay ready for review.</h1><p>Manage hospital records, ownership, and accreditation evidence in one focused workspace.</p><div className="login-note"><FileText size={16} /><span>Master list of documents</span></div></section><form className="login-panel" onSubmit={showForgotPassword ? requestReset : submit}><img src={hospitalLogo} alt="NABH Docs" className="login-logo" /><p className="eyebrow">Secure workspace</p><h2>{showForgotPassword ? "Reset your password" : "Sign in"}</h2>{showForgotPassword ? <><p className="login-form-copy">Enter your email or user ID and we will send the password reset link to your registered email address.</p><label>Email or User ID<input value={resetIdentifier} onChange={(event) => setResetIdentifier(event.target.value)} autoComplete="username" autoFocus required /></label>{error && <p className="status error">{error}</p>}{resetMessage && <p className="access-message">{resetMessage}</p>}<button className="login-submit" disabled={resetting}><KeyRound size={16} /> {resetting ? "Sending link..." : "Send reset link"}</button><button type="button" className="login-back-button" onClick={() => { setShowForgotPassword(false); setError(""); setResetMessage(""); }}><ArrowLeft size={15} /> Back to sign in</button></> : <><label>User ID or email<input value={userId} onChange={(event) => setUserId(event.target.value)} autoComplete="username" /></label><label>Password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" /></label>{error && <p className="status error">{error}</p>}<button className="login-submit">Continue <ArrowRight size={16} /></button><button type="button" className="login-forgot-button" onClick={() => { setShowForgotPassword(true); setError(""); }}>Forgot password?</button>{onShowRegister && <button type="button" className="login-submit" style={{ background: "transparent", color: "#3547d1" }} onClick={onShowRegister}>New hospital? Register here</button>}<div className="example-logins"><p>Example accounts</p>{exampleAccounts.map(([label, id, email, secret]) => <button type="button" key={id} onClick={() => { setUserId(id); setPassword(secret); setError(""); }}><span>{label}</span><code>{id}{email && ` · ${email}`}</code></button>)}</div><p className="login-hint">Hospital administrator passwords use <code>Hospital@123</code>.</p></>}</form></main>;
}