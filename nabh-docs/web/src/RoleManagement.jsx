...
export default function RoleManagement({ hospitalId, hospitalName, hospitalLogoPath }) {
  ...
  async function matrix(role, action) { if (isReadOnly) return setMessage("Hospital is not yet onboarded. Changes are disabled until a Super Admin approves onboarding."); const permissions = (role.permissions || ["view"]).includes(action) ? role.permissions.filter((item) => item !== action) : [...(role.permissions || ["view"]), action]; await fetch(`/api/admin/hospitals/${hospitalId}/roles/${role.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...role, permissions: permissions.some((item) => item !== "view") && !permissions.includes("view") ? ["view", ...permissions] : permissions }) }); await load(); }
